...
const handleVerifyOTP = async () => {
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 px-4 py-12 text-slate-900 dark:text-slate-100">
      ...
      <div className="max-w-5xl w-full">
        ...
        <div className="flex justify-center mb-8 gap-4">
          <div className={`h-2 w-12 rounded-full transition-colors ${step === 'register' ? 'bg-emerald-600' : 'bg-emerald-200 dark:bg-emerald-900'}`}></div>
          <div className={`h-2 w-12 rounded-full transition-colors ${step === 'specialization' ? 'bg-emerald-600' : (step === 'verify' || step === 'plan') ? 'bg-emerald-200 dark:bg-emerald-900' : 'bg-slate-200 dark:bg-slate-800'}`}></div>
          <div className={`h-2 w-12 rounded-full transition-colors ${step === 'verify' ? 'bg-emerald-600' : step === 'plan' ? 'bg-emerald-200 dark:bg-emerald-900' : 'bg-slate-200 dark:bg-slate-800'}`}></div>
          <div className={`h-2 w-12 rounded-full transition-colors ${step === 'plan' ? 'bg-emerald-600' : 'bg-slate-200 dark:bg-slate-800'}`}></div>
        </div>

        {/* STEP 1: REGISTRATION FORM */}
        {step === 'register' && (
          <div className="max-w-2xl mx-auto bg-white dark:bg-slate-900 rounded-3xl shadow-xl p-8 border border-slate-100 dark:border-slate-800 animate-in fade-in slide-in-from-bottom-4">
            <div className="text-center mb-8">
              <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Create your Account</h2>
              <p className="text-slate-500 dark:text-slate-400 text-sm">Join thousands of students acing their exams.</p>
            </div>
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleCategoryNext(); }} className="space-y-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    type="text" required 
                    className="w-full px-4 py-3 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-950 text-slate-900 dark:text-white focus:ring-2 focus:ring-emerald-500 outline-none transition"
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-3">
                  What type of exam are you preparing for? <span className="text-red-500 dark:text-red-400">*</span>
                </label>
                        ...
                        return (
                          <button
                            ...
                            className={`
                              ...
                              ${isDisabled
                                ? 'opacity-60 cursor-not-allowed border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50'
                                : isSelected
                          ...
                          >
                            ...
                            <h4 className={`font-bold text-slate-900 dark:text-white text-sm mb-1 ${isDisabled ? 'opacity-70' : ''}`}>
                              {category.label}
                            </h4>
                            <p className={`text-xs text-slate-500 dark:text-slate-400 leading-relaxed ${isDisabled ? 'opacity-70' : ''}`}>
                              {category.description}
                            </p>

                            <div className="flex flex-wrap gap-1 mt-3">
                              {category.exams.slice(0, 2).map((exam, idx) => (
                                <span key={idx} className={`text-[9px] bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-2 py-0.5 rounded-full ${isDisabled ? 'opacity-50' : ''}`}>
                                  {exam}
                                </span>
                              ))}
                            </div>
              ...
              <button 
                ...
                className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3.5 rounded-xl transition flex items-center justify-center shadow-lg shadow-emerald-200 dark:shadow-emerald-900/30 disabled:bg-emerald-400"
              >
            ...
            <p className="mt-6 text-center text-sm text-slate-600 dark:text-slate-400">
              Already have an account? <Link href="/login" className="text-emerald-600 dark:text-emerald-400 font-bold hover:underline">Log in</Link>
            </p>
          </div>
        )}
        ...
        {step === 'specialization' && (
          <div className="max-w-3xl mx-auto bg-white dark:bg-slate-900 rounded-3xl shadow-xl p-8 border border-slate-100 dark:border-slate-800 animate-in fade-in slide-in-from-right-4">
            <div className="text-center mb-8">
              <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Choose Your Specialization</h2>
              <p className="text-slate-500 dark:text-slate-400 text-sm">Select your subject area to get personalized content</p>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`
                      ...
                      ${isSelected 
                        ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-950/40 shadow-xl scale-105' 
                        : 'border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 bg-white dark:bg-slate-900'
                      }
                    `}
                  >
                    ...
                    <h4 className="font-bold text-slate-900 dark:text-white mb-2">{spec.label}</h4>
                    <p className="text-sm text-slate-500 dark:text-slate-400 mb-3">{spec.description}</p>
                    
                    <div className="flex flex-wrap gap-1">
                      {spec.subjects.slice(0, 4).map((subject, idx) => (
                        <span key={idx} className="text-[10px] bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-2 py-1 rounded-full">
                          {subject}
                        </span>
                      ))}
                      {spec.subjects.length > 4 && (
                        <span className="text-[10px] bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-2 py-1 rounded-full">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="px-6 py-3 border-2 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 font-bold rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 transition"
              >
                Back
              </button>
              <button
                ...
                className="flex-1 bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-400 text-white font-bold py-3 rounded-xl transition flex items-center justify-center shadow-lg shadow-emerald-200 dark:shadow-emerald-900/30"
              >
...
{step === 'verify' && (
  <div className="max-w-2xl mx-auto bg-white dark:bg-slate-900 rounded-3xl shadow-xl p-8 border border-slate-100 dark:border-slate-800 animate-in fade-in slide-in-from-right-4">
    <div className="text-center mb-8">
      ...
      <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Enter Verification Code</h2>
      <p className="text-slate-500 dark:text-slate-400 text-sm">
        We've sent a 6-digit code to <strong className="text-slate-900 dark:text-white">{email}</strong>
      </p>
    </div>
    ...
    <div className="space-y-6">
      <div>
        <input 
          ...
          className="w-full text-center text-3xl font-bold tracking-[1rem] py-4 rounded-xl border-2 border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-950 text-slate-900 dark:text-white focus:border-emerald-500 focus:ring-4 focus:ring-emerald-50 dark:focus:ring-emerald-950/50 outline-none transition"
        />
      </div>
      ...
      <button
        ...
        className="w-full bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-400 text-white font-bold py-4 rounded-xl transition flex items-center justify-center gap-2 shadow-lg shadow-emerald-200 dark:shadow-emerald-900/30"
      >
      ...
      <button
        ...
        className="w-full text-emerald-600 dark:text-emerald-400 font-medium py-2 hover:underline flex items-center justify-center gap-2"
      >
...
{step === 'plan' && (
  <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-right-8 duration-500">
    <div className="text-center mb-10">
      <div className="inline-flex items-center gap-2 bg-emerald-100 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-300 px-4 py-2 rounded-full text-sm font-bold mb-4">
        <Star size={16} fill="currentColor" /> Email Verified Successfully!
      </div>
      <h2 className="text-3xl font-bold text-slate-900 dark:text-white">Choose a Starting Credit Pack</h2>
      <p className="text-slate-500 dark:text-slate-400">Credits are used to take exams. Select a pack to get started.</p>
    </div>
        ...
        <button
          ...
          className={`
            ...
            ${selectedPackage.id === pkg.id 
              ? 'border-emerald-500 bg-white dark:bg-slate-900 shadow-xl scale-105 ring-4 ring-emerald-50 dark:ring-emerald-950/50' 
        ...
        >
          ...
          <div className="space-y-3 mb-8 flex-grow">
            <div className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400">
              ...
              <span className="font-bold text-slate-900 dark:text-white">{pkg.credits} Credits</span>
            </div>
            {pkg.bonus > 0 && (
    ...
    <div className="max-w-md mx-auto">
      <button
        ...
        disabled={loading}
        className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-4 rounded-2xl transition flex items-center justify-center gap-3 shadow-xl shadow-emerald-200 dark:shadow-emerald-900/30 disabled:bg-emerald-400 text-lg"
      >
        {loading ? (
