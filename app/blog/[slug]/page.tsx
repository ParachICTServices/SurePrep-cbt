...
const ptComponents = {
  block: {
    normal: ({ children }: any) => (
      <p className="text-slate-300 leading-[1.85] mb-6 text-[15px]">{children}</p>
    ),
    h2: ({ children }: any) => (
      <h2 className="text-2xl font-bold text-white mt-14 mb-5 pb-3 border-b border-white/5">{children}</h2>
    ),
    h3: ({ children }: any) => (
      <h3 className="text-xl font-bold text-white mt-10 mb-4">{children}</h3>
    ),
    blockquote: ({ children }: any) => (
      <blockquote className="relative border-l-4 border-emerald-500 pl-6 pr-4 py-2 my-8 bg-emerald-500/5 rounded-r-xl">
  ...
  listItem: {
    bullet: ({ children }: any) => (
      <li className="text-slate-300 flex items-start gap-3 text-[15px]">
        <span className="mt-[9px] h-1.5 w-1.5 rounded-full bg-emerald-400 shrink-0" />
  ...
  marks: {
    ...
    code: ({ children }: any) => (
      <code className="px-2 py-0.5 bg-slate-800 border border-white/10 rounded text-emerald-300 text-sm font-mono">
        {children}
      </code>
    ),
  },
  ...
  <Link href={`/blog/${article.slug}`} className="group block">
    <div className="flex gap-4 py-4 border-b border-white/5 last:border-0 hover:bg-white/[0.02] rounded-xl px-3 -mx-3 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-slate-200 leading-snug mb-1.5 group-hover:text-emerald-300 transition-colors line-clamp-2">
          {article.title}
        </p>
      ...
      <ArrowRight size={14} className="shrink-0 text-slate-700 group-hover:text-emerald-400 mt-1 group-hover:translate-x-0.5 transition-all" />
    </div>
  </Link>
)
    ...
    <div className="min-h-screen bg-slate-950 text-white font-sans">
      ...
      <nav className="fixed w-full z-50 bg-slate-950/80 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex gap-4">
            ...
            <Link href="/register" className="px-5 py-2.5 text-sm font-bold bg-emerald-500 text-white rounded-full hover:bg-emerald-400 transition-all active:scale-95">
              Get Started
            </Link>
          </div>
        </div>
      ...
      <main className="pt-24 pb-24">
        <div className="max-w-7xl mx-auto px-6">
          ...
          {/* Back link */}
          <div className="py-6">
            <Link href="/blog" className="inline-flex items-center gap-2 text-slate-400 hover:text-emerald-400 transition-colors text-sm font-medium">
            ...
            <div className="min-w-0">
              ...
              <header className="mb-10">
                <div className="flex flex-wrap items-center gap-3 mb-6">
                  <span className={`text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-full border ${categoryColors[article.category] ?? 'bg-white/5 text-slate-400 border-white/10'}`}>
                    {article.category}
                  </span>
                ...
                <p className="text-lg text-slate-400 leading-relaxed border-l-4 border-emerald-500/40 pl-5 bg-emerald-500/[0.03] py-3 rounded-r-xl">
                  {article.excerpt}
                </p>

                <div className="mt-10 h-px bg-gradient-to-r from-emerald-500/30 via-white/5 to-transparent" />
              </header>
              ...
              <div className="lg:hidden mt-12 space-y-6">
                {related?.length > 0 && (
                  <div className="bg-slate-900 border border-white/5 rounded-2xl p-6">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h2 className="text-xs font-bold uppercase tracking-widest text-slate-400">More in {article.category}</h2>
                    </div>
                ...
                {latest?.length > 0 && (
                  <div className="bg-slate-900 border border-white/5 rounded-2xl p-6">
                    <h2 className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-4">Latest Articles</h2>
              ...
              <div className="sticky top-24 space-y-5">
                ...
                {related?.length > 0 && (
                  <div className="bg-slate-900 border border-white/5 rounded-2xl p-6">
                    ...
                    <p className="text-[10px] text-slate-600 mb-4 pl-4">Similar articles you might find useful</p>
                    {related.map((a: SidebarArticle) => <SidebarCard key={a._id} article={a} />)}
                    <Link
                      href={`/blog`}
                      className="mt-4 flex items-center gap-1.5 text-xs text-emerald-500 hover:text-emerald-400 font-semibold transition-colors"
                    >
                ...
                {latest?.length > 0 && (
                  <div className="bg-slate-900 border border-white/5 rounded-2xl p-6">
                    <h2 className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1">Latest Articles</h2>
                    <p className="text-[10px] text-slate-600 mb-4">What's been published recently</p>
                    {latest.map((a: SidebarArticle) => <SidebarCard key={a._id} article={a} />)}
                    <Link
                      href="/blog"
                      className="mt-4 flex items-center gap-1.5 text-xs text-emerald-500 hover:text-emerald-400 font-semibold transition-colors"
                ...
                {/* Mini CTA */}
                <div className="bg-slate-900 border border-emerald-500/10 rounded-2xl p-6">
                  <p className="text-xs font-bold uppercase tracking-widest text-emerald-500 mb-3">Sure Prep CBT</p>
                  <p className="text-sm text-slate-200 font-semibold leading-snug mb-2">
