    ...
    <div className="p-6 text-slate-100 max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          {id && <p className="text-slate-400 font-mono text-sm mt-0.5">{id}</p>}
        </div>
      </div>
      ...
      <div className="bg-slate-800/80 rounded-xl border border-slate-700 overflow-hidden">
        {loading ? (
          ...
          <p className="text-center text-slate-400 py-16">Transaction not found.</p>
        ) : (
          <dl className="divide-y divide-slate-700">
            {entries.map(([key, value]) => (
              ...
              >
                <dt className="text-xs font-medium text-slate-500 uppercase tracking-wide sm:col-span-1">
                  {key}
                </dt>
                <dd className="mt-1 text-sm text-slate-200 sm:col-span-2 sm:mt-0 break-words whitespace-pre-wrap font-mono">
                  {formatValue(value)}
                </dd>
              </div>
            ))}
